...
| `0.3.0` | Dataset OpenCUP statico + lookup browser | PoC indice CUP statico, pipeline mensile, indice CUP esatto su GitHub Releases, verifica esistenza CUP da web e Python, cache locale/browser, nuovi esiti OpenCUP | nessuna |
...
3. Indice ottimizzato `WITHOUT ROWID`, con chiave `cup` e riferimento opzionale al chunk dettagli.
4. Indice CUP esatto pubblicato su GitHub Releases e descritto da `dataset-manifest.json`.
5. Chunk dell'indice scaricabili e cacheabili dal browser senza servizi server-side.
6. Integrazione web: lookup locale nel browser sui CUP unici, con fallback a `FORMATO_VALIDO_DA_VERIFICARE` se il dataset non e disponibile.
...
10. ADR per strategia statica GitHub Releases + browser lookup (ADR 0007) e PoC indice statico (ADR 0008).
